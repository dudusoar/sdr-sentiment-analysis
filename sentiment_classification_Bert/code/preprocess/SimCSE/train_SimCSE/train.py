...
def parse_args():
    ...
    parser.add_argument(
        "--train_file", type=str, default="./data/train_test.txt" , help="train text file"
    )
    parser.add_argument(
        "--PTM_root_path", type=str, default="/", help="the root path of PTMs"
    )
    parser.add_argument(
        "--pretrained", type=str, default='/home/ywc/pretrained_model/bert-base-uncased/', help="huggingface pretrained model"
    )
    parser.add_argument(
        "--num_proc", type=int, default=1, help="dataset process thread num"
    )
    parser.add_argument(
        "--max_length", type=int, default=128, help="sentence max length"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--learning_rate", type=float, default=5e-6, help="learning rate,1e-5"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--save_final", type=bool, default=True, help="save the final model"
    )
    parser.add_argument(
        "--pool_type", type=str, default="cls", help="pool_type"
    )
    parser.add_argument(
        "--dropout_rate", type=float, default=0.1, help="dropout_rate"
    )
    parser.add_argument(
...
def main():
    ...
    model_out = './ckpts/{}_{}'.format(args.pretrained,data_lang)
    make_dir(model_out)
