...
class PriorMultiLabelSoftMarginLoss(nn.Module):
    def __init__(self, prior=None, num_labels=None, reduction="mean", eps=1e-9, tau=1.0):
        ...
        urls: [Use mutual information concept to alleviate class imbalance](https://spaces.ac.cn/archives/7615)
        args:
            prior: List<float>, prior of label, prior knowledge.  eg. [0.6, 0.2, 0.1, 0.1]
            num_labels: int, num of labels, number of classes.  eg. 10
            reduction: str, Specifies the reduction to apply to the output, output format.
                            eg.``'none'`` | ``'mean'`` | ``'sum'``. ``'none'``
            eps: float, Minimum of maths, minimum value.  eg. 1e-9
            tau: float, weight of prior in loss, weight of prior knowledge, eg. ``1.0``
        returns:
            Tensor of loss.
        """
        super(PriorMultiLabelSoftMarginLoss, self).__init__()
        self.loss_mlsm = torch.nn.MultiLabelSoftMarginLoss(reduction=reduction)
        if not prior: prior = np.array([1/num_labels for _ in range(num_labels)])  # If not exists, set to uniform distribution
